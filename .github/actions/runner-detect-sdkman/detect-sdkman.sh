...
source "${SDKMAN_INIT:-}" && printf '::notice title=SDKMAN is sourced at:sdkman_init=%s, requesting version=%s, and native=%s\n' "${SDKMAN_INIT:-}" "$floor_sdkman" "$floor_native" || exit 0
